...
solution GeneticAlgorithm::run(int numberOfBits, Point low, Point high)
{
...
std::vector<solution> GeneticAlgorithm::tournamentCrossover(std::vector<solution> const& currentGeneration)
{
	...
	while (newSolutions.size() < mPopulationSize)
	{
		...
		//perform crossover
		std::vector<solution> children = winner1.MultiPointCrossover(winner2, mCrossoverProbability);
